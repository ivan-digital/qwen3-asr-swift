...
public final class SortformerDiarizer {
    ...
    /// Chunk index (0 = first chunk, affects left context)
    ...
    public func diarize(
        ...
        if sampleRate != self.config.sampleRate {
            samples = resample(audio, from: sampleRate, to: self.config.sampleRate)
        } else {
            samples = audio
        }
        ...
        while endFeat < totalMelFrames {
            ...
            do {
                ...
                // Skip chunk on inference failure
            }
    ...
    private func binarizeCorePredictions(
        ...
        return mergeAdjacentSegments(allSegments, minSilence: minSilenceDuration)
    }
    ...
    private func mergeAdjacentSegments(
        ...
        // Compact speaker IDs
        let usedIds = Set(merged.map(\.speakerId)).sorted()
