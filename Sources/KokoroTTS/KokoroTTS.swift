...
public final class KokoroTTSModel {
    ...
    public func synthesize(
        ...
        return try synthesizeE2E(tokenIds: tokenIds, styleVector: styleVector, speed: speed)
    }

    // MARK: - E2E Inference

    private func synthesizeE2E(tokenIds: [Int], styleVector: [Float], speed: Float) throws -> [Float] {
        guard let network else { throw AudioModelError.inferenceFailed(operation: "kokoro-e2e", reason: "Not loaded") }
        let padTo = 128
        let tokenCount = min(tokenIds.count, padTo)
